...
use RedisLock\LockInterface;

class IdempotencyRedisLock implements LockInterface {
    ...
    const LUA_SCRIPT_UPDATE_LOCK_VALUE = '
        local ttl = redis.call("ttl", ARGV[1]) if ttl > 0 then
     ...
     * @throws \RedisClient\Exception\ErrorResponseException
     */
    ...
    public function updateToken(string $key, string $token)
    {
        return $this->Redis->eval(self::LUA_SCRIPT_UPDATE_LOCK_VALUE, null, [$key,$token]);
    }
